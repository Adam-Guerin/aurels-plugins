...
| --- | --- | --- | --- | --- |
| Aurels OpenClaw | 0.2.4 | Node 20+, OpenClaw 2026.3.2+ | Supported | `before_tool_call` / `after_tool_call` only |
| Aurels Hermes | 0.2.4 | Python 3.11+, native hook host | Supported | `pre_tool_call` / `post_tool_call` only |
| Aurels Ollama | 0.2.4 | Ollama with `qwen3-coder:q3` | Supported | Local analysis only; not an execution hook |
